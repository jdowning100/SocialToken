...
registerSetting('forum.postInterval', 1, 'How long users should wait between each posts, in seconds');
registerSetting('forum.maxPostsPerDay', 50, 'Maximum number of posts a user can create in a day');
...
function PostsNewRateLimit (post, user) {
  ...
  if(!Users.isAdmin(user)){
    ...
    var timeSinceLastPost = Users.timeSinceLast(user, Posts),
      ...
      postInterval = Math.abs(parseInt(getSetting('forum.postInterval', 1))),
      maxPostsPer24Hours = Math.abs(parseInt(getSetting('forum.maxPostsPerDay', 50)));
