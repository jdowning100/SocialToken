...
registerSetting('forum.commentInterval', 1, 'How long users should wait in between comments (in seconds)');

function CommentsNewRateLimit (comment, user) {
  if (!Users.isAdmin(user)) {
    const timeSinceLastComment = Users.timeSinceLast(user, Comments);
    const commentInterval = Math.abs(parseInt(getSetting('forum.commentInterval',1)));
